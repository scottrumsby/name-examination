          ...
          <tr :style="index % 2 === 0 ? { backgroundColor: 'white' } : { backgroundColor: 'var(--xl-grey)' }">
            <td class="py-2">
              <v-container fluid ma-0 pa-0>
                ...
                <v-layout v-for="(name, i) in item.names" wrap>
                  <v-flex fs-14 style="font-weight: 600" :key="`1st-flex-${i}`" lg12 ml-2 mb-2>
                    {{ i + 1 }}. {{ name.name }}<span :class="getClass(name.state)" class="mx-2">{{ name.state }}</span>
                  </v-flex>
                  <v-layout fs-13 column>
                    <v-flex mb-2 v-for="text in name.decision_text" ml-4 ft-ital>{{ text }}</v-flex>
                  </v-layout>
                </v-layout>
                <v-layout v-if="item.comments.length > 0" ml-2 mt-1 dk-grey fs-14 fw-600>LAST COMMENT</v-layout>
                <v-layout v-if="item.comments.length > 0" dk-grey ml-2 mb-2 wrap>
