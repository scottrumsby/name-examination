...
<template>
  <v-card flat
    ...
    <v-container fluid p-1 ma-0>
      <v-layout mt-4>
        <v-flex lg8="!is_editing"><v-icon color="grey" class="mirrored fs-18 ml-3 mr-1">
          chat_bubble_outline </v-icon>{{ commentsCount }} Comments</v-flex>
        <v-flex lg4 text-right>
          <v-btn class="ma-0 pa-0"
                 flat
                 :ripple="false"
                 @click="toggleCommentsPopUp(false)">
            <v-icon :style="alignTop" color="light-blue">clear</v-icon><span :style="alignTop" color="blue">Close
            </span>
          </v-btn>
        </v-flex>
      </v-layout>
    <v-layout column>
      <v-flex><v-textarea class="comments-text-area"
                          no-resize
                          v-model="newComment" /></v-flex>
    </v-layout>
