        ...
        <v-card flat
            ...
            <v-layout column ma-0 pa-0>
              ...
              <template v-if="infoType === 'information' ">
                <v-flex lg12 v-if="is_editing || (isActivePopUp && is_my_current_nr)">
                  <v-textarea class="addtnl-info-text-area"
                              v-model="additionalInfo"/>
                </v-flex>
                <v-flex v-if="!is_expanded && isActivePopUp && is_my_current_nr" text-right lg12 c-link>
                ...
                <v-flex v-if="(isActivePopUp && !is_my_current_nr) || is_viewing" lg12>
                  {{ text }}
                </v-flex>
              </template>
              <template v-if="infoType === 'nature'">
                <v-flex px-1 lg-12 v-if="is_expanded || isActivePopUp">
                  {{ text }}
                </v-flex>
                <v-flex lg12 v-if="!is_expanded && !isActivePopUp">
                  {{ truncate(text) }}
