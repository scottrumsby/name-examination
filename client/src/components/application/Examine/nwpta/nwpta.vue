...
<template>
  <fragment>
    <template v-if="is_editing && has_nwpta && (!is_numbered_assumed || nwpta_requested)">
      ...
      <template v-if="!nwpta_requested || requested_radio_selection_nuans || requested_radio_selection_assumed"
        <v-flex :class="{'form-group-error': $v.nwpta.partnerNameDate.$error}"
                align-self-center>
          <v-text-field v-model="nwpta.partnerNameDate"
                        class="nwpta-field"
                        placeholder="Expiry Date"
                        @input="$v.nwpta.partnerNameDate.$touch()" />
        </v-flex>
        <v-flex ft-ital text-center>YYYY-MM-DD</v-flex>
    ...
    <v-flex fs-14 v-else-if="!is_editing && nwpta_requested && has_nwpta">
      <b>{{ jurisdiction }}: </b>Requested
    </v-flex>

    <template v-else-if="(!is_editing || is_numbered_assumed) && has_nwpta && !nwpta_requested">
      <v-flex fs-14><b>{{ jurisdiction }}: </b>{{ is_named_assumed ? '(Assumed)' : '' }}
                                         {{ nwpta_requested ?  'Requested' : ''}}</v-flex>
      <v-flex v-if="is_numbered_assumed" fw-600>Number Assumed: </v-flex>
      <!-- number -->
