  ...
  <v-container decision-container fluid py-0 px-3 :opacity-30="!is_making_decision">
    <v-layout wrap>
      <v-flex lg6
              fs-24
              grow
              style="position: relative; top: 10px;">Decision</v-flex>
      <v-layout justify-end>
        ...
        <div class="fs-15" style="position: relative; top: 14px">Consent </div>
      </v-layout>

      <v-flex lg12><v-divider /></v-flex>
...
/* eslint-disable */
  ...
  export default {
    ...
    computed: {
      ...
      consent_required() {
        if (this.consent_required_by_condition || this.consent_required_by_user) return true
        return false
      },
      consent_required_by_condition() {
        if (this.selected_conditions && Array.isArray(this.selected_conditions)) {
          return this.selected_conditions.some(condition => condition.consent_required_tf)
        }
        return false
      },
      currentNameObj: {
